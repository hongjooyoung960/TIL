...
2. **FastAPI**는 SQLAlchemy 세션으로 PostgreSQL을 읽고 씁니다. Neon 같은 **호스티드 Postgres**를 쓰면 로컬 DB 설치 없이 `DATABASE_URL`만으로 동일 코드를 실행할 수 있습니다. 일간 계획 저장 및 활동 변경 시 **성취도(가중치)**를 다시 계산합니다.
